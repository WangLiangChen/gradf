...
public class RingBuffer {
    ...
    public RingBuffer(int bufferSize, int paddingFactor) {
        ...
        Assert.INSTANCE.isTrue(bufferSize > 0L, AssertLevel.INFO, "RingBuffer size must be positive");
        Assert.INSTANCE.isTrue(Integer.bitCount(bufferSize) == 1, AssertLevel.INFO, "RingBuffer size must be a power of 2");
       Assert.INSTANCE.isTrue(paddingFactor > 0 && paddingFactor < 100, AssertLevel.INFO, "RingBuffer size must be positive");
    ...
    /**
     * Take an UID of the ring at the next cursor, this is a dblock free operation by using atomic cursor<p>
     * 
    ...
    public long take() {
        ...
        Assert.INSTANCE.isTrue(nextCursor >= currentCursor, AssertLevel.INFO, "Curosr can't move back");
        ...
        Assert.INSTANCE.isTrue(flags[nextCursorIndex].get() == CAN_TAKE_FLAG, AssertLevel.INFO, "Curosr not in can take status");
