...
public class RingBuffer {
    ...
    public RingBuffer(int bufferSize, int paddingFactor) {
        ...
        Assert.INSTANCE.isTrue(bufferSize > 0L, "RingBuffer size must be positive");
        Assert.INSTANCE.isTrue(Integer.bitCount(bufferSize) == 1, "RingBuffer size must be a power of 2");
        Assert.INSTANCE.isTrue(paddingFactor > 0 && paddingFactor < 100, "RingBuffer size must be positive");
    ...
    public long take() {
        ...
        Assert.INSTANCE.isTrue(nextCursor >= currentCursor, "Curosr can't move back");
        ...
        Assert.INSTANCE.isTrue(flags[nextCursorIndex].get() == CAN_TAKE_FLAG, "Curosr not in can take status");
